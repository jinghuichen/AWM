...
class VGG(nn.Module):
    ...
    def __init__(self, features, num_classes=43):
        ...
        self.classifier = nn.Linear(2048, num_classes) # vgg_small
...
def make_layers(cfg, batch_norm=False, norm_layer=None):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                layers += [conv2d, norm_layer(v), nn.ELU(inplace=True)]
            else:
...
cfg = {
    ...
    'small_VGG16': [32, 32, 'M', 64, 64, 'M', 128, 128, 'M']
}
...
def small_vgg(norm_layer=nn.BatchNorm2d, **kwargs):
    model = VGG(make_layers(cfg['small_VGG16'], batch_norm=True, norm_layer=norm_layer), **kwargs)
    return model