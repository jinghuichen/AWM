...
class BasicBlock(nn.Module):
    def __init__(self, in_channels, norm_layer, split_ratio=0.5):
        super(BasicBlock, self).__init__()
        self.split = SplitBlock(split_ratio)
        in_channels = int(in_channels * split_ratio)
        self.conv1 = MaskedConv2d(in_channels, in_channels, kernel_size=1, bias=False)
        ...
        self.conv3 = MaskedConv2d(in_channels, in_channels, kernel_size=1, bias=False)
...
class DownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer):
        ...
        self.bn1 = norm_layer(in_channels)
        self.conv2 = MaskedConv2d(in_channels, mid_channels, kernel_size=1, bias=False)
        self.bn2 = norm_layer(mid_channels)
        # right
        self.conv3 = MaskedConv2d(in_channels, mid_channels, kernel_size=1, bias=False)
        ...
        self.bn4 = norm_layer(mid_channels)
        self.conv5 = MaskedConv2d(mid_channels, mid_channels, kernel_size=1, bias=False)
        self.bn5 = norm_layer(mid_channels)
...
class ShuffleNetV2(nn.Module):
    def __init__(self, net_size, norm_layer = nn.BatchNorm2d):
        super(ShuffleNetV2, self).__init__()
        out_channels = configs[net_size]["out_channels"]
        num_blocks = configs[net_size]["num_blocks"]

        self.conv1 = MaskedConv2d(3, 24, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.conv2 = MaskedConv2d(out_channels[2], out_channels[3], kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = norm_layer(out_channels[3])
        self.linear = nn.Linear(out_channels[3], 10)

    def _make_layer(self, out_channels, num_blocks, norm_layer):
        layers = [DownBlock(self.in_channels, out_channels, norm_layer)]
        for i in range(num_blocks):
            layers.append(BasicBlock(out_channels, norm_layer))
